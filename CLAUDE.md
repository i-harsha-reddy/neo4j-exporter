...
- **`SHOW TRANSACTIONS` returns `elapsedTime` as a Cypher Duration**, which the v5 Go driver surfaces as `time.Duration`. The `asFloat64` helper in `internal/collector/coerce.go` already handles this — preserve that branch when extending.
